...
export default function PaperDetailsPage() {
  ...
  const handleReviewSubmit = (e: React.FormEvent) => {
    ...
    setReviews([
      {
        author: user?.name || "Anonymous Researcher",
        rating: newRating,
        text: newComment
      },
      ...reviews
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-8 pb-32 space-y-12">
      ...
      {/* Main Page Layout Layout Splits */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
        
        {/* LEFT COLUMN: Main Research Core & Reviews */}
        <div className="lg:col-span-8 space-y-10">
          <div>
            <span className="font-mono text-xs text-[#8A1A1A] tracking-widest uppercase block mb-3">// PAPER ID: {PAPER_DATA.id}</span>
          ...
          <section className="space-y-6">
            ...
            {/* Check if user is logged in before letting them type a review */}
            {isLoggedIn ? (
              <form onSubmit={handleReviewSubmit} className="glass-card p-6 rounded-lg border border-[#8A1A1A]/20 space-y-4">
                <span className="font-mono text-[10px] text-[#E9D4C3] tracking-widest block uppercase">// WRITE A REVIEW</span>
                
                <div className="flex items-center space-x-4">
                  <label className="font-mono text-xs text-[#7C8FA9]">Your Rating [1-5]:</label>
                  <select 
                    value={newRating}
                ...
                <textarea
                  ...
                  placeholder="Share your thoughts or findings about this paper..."
              ...
              <div className="glass-card p-4 rounded-lg border border-[#7C8FA9]/10 flex items-center space-x-3">
                ...
                <p className="font-mono text-[11px] text-[#7C8FA9]">
                  Please <span className="text-[#E9D4C3] font-bold">Sign In</span> to leave a review or score this document.
                </p>
              </div>
            )}
              ...
              {reviews.map((rev, idx) => (
                <div key={idx} className="glass-card p-5 rounded-lg border border-[#E9D4C3]/5 space-y-2">
                  <div className="flex items-center justify-between font-mono text-[10px]">
                    <span className="text-[#E9D4C3]">// BY: {rev.author}</span>
        ...
        {/* RIGHT COLUMN: Key Metadata Record Dashboard */}
        <div className="lg:col-span-4">
          <div className="glass-card p-6 rounded-lg border border-[#E9D4C3]/10 font-mono text-xs space-y-4 sticky top-28">
            <h3 className="text-[#E9D4C3] font-display text-sm font-bold tracking-tight mb-2 uppercase">// DOCUMENT SUMMARY</h3>
            <div className="space-y-3 pt-2">
              ...
              <div className="flex justify-between items-center border-b border-[#E9D4C3]/5 pb-2">
                <span className="text-[#7C8FA9] text-[10px] uppercase">Published:</span>
                <span className="text-white font-bold">{PAPER_DATA.year}</span>
              </div>
              <div className="flex justify-between items-center border-b border-[#E9D4C3]/5 pb-2">
                <span className="text-[#7C8FA9] text-[10px] uppercase">Research Field:</span>
                <span className="text-[#E9D4C3] bg-[#8A1A1A]/20 px-2 py-0.5 rounded font-bold">{PAPER_DATA.field}</span>
              </div>
              <div className="flex justify-between items-center border-b border-[#E9D4C3]/5 pb-2">
                <span className="text-[#7C8FA9] text-[10px] uppercase">Citations:</span>
                <span className="text-white font-bold">{PAPER_DATA.cites}</span>
              </div>
              <div className="flex flex-col gap-1.5 pt-1">
                <span className="text-[#7C8FA9] text-[10px] uppercase">Index Keywords:</span>
