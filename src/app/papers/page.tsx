...
import { Search, Layers, ArrowUpDown, ChevronLeft, ChevronRight, ArrowUpRight } from "lucide-react";
...
export default function ExplorePage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-8 pb-24">
      ...
      <div className="mb-10">
        <span className="font-mono text-[10px] text-[#7C8FA9] tracking-widest block mb-1">// SYS_INDEX_QUERY_TERM</span>
        <h1 className="font-display text-3xl font-bold text-white tracking-tight">Centralized Document Matrix</h1>
      </div>
      ...
      <div className="glass-card p-6 rounded-lg border border-[#E9D4C3]/10 mb-8 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
          <div className="md:col-span-5 relative">
            ...
            <input 
              ...
              className="w-full bg-[#0A1626] border border-[#E9D4C3]/15 rounded pl-10 pr-4 py-2 text-xs font-mono text-white focus:outline-none focus:border-[#8A1A1A]"
            />
          </div>
          ...
          <div className="md:col-span-3 relative">
            ...
            <select
              ...
              className="w-full bg-[#0A1626] border border-[#E9D4C3]/15 rounded pl-10 pr-4 py-2 text-xs font-mono text-[#E9D4C3] focus:outline-none appearance-none"
            >
        ...
        <div className="pt-2 border-t border-[#E9D4C3]/5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <span className="font-mono text-[10px] text-[#7C8FA9]">HITS: {sortedPapers.length} / {SEED_PAPERS.length}_NODES</span>
        </div>
      </div>
          ...
          {sortedPapers.map((paper) => (
            ...
            >
              ...
              <div>
                ...
                <span className="font-mono text-[10px] text-[#7C8FA9] tracking-wider uppercase block mb-1.5">// {paper.field}</span>
                <h3 className="font-display text-sm font-bold text-white tracking-tight leading-snug line-clamp-2 group-hover:text-[#E9D4C3] transition-colors">{paper.title}</h3>
                <p className="font-sans text-[11px] text-[#7C8FA9] mt-2 line-clamp-3 leading-relaxed">{paper.desc}</p>
              </div>
              ...
              <div className="mt-6 pt-3 border-t border-[#E9D4C3]/5 flex items-center justify-between font-mono text-[10px] text-[#7C8FA9]">
                ...
                {/* 🔄 FIXED: Converted from a generic button to a Next.js dynamic routing Link */}
                <Link 
      ...
      <div className="mt-12 pt-6 border-t border-[#E9D4C3]/5 flex items-center justify-between font-mono text-xs text-[#7C8FA9]">
        ...
        <button 
          disabled={true}
          className="flex items-center space-x-1 px-3 py-1.5 border border-[#E9D4C3]/10 rounded opacity-30 cursor-not-allowed"
        >
          <span>NEXT</span>
