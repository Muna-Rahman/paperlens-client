...
import StatStamp from '@/components/ui/StatStamp';
        ...
        {loading ? (
          ...
          <div className="relative">
            <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 ${!isAuthenticated ? 'select-none pointer-events-none mix-blend-screen opacity-40 blur-[1px]' : ''}`}>
              {/* FIXED: added index parameters here cleanly */}
              {papers.map((paper, index) => (
                <Link
                  key={paper._id}
                  href={`/papers/${paper._id}`}
                  className="group backdrop-blur-[16px] bg-[rgba(233,212,195,0.06)] hover:bg-[rgba(233,212,195,0.09)] border border-[rgba(233,212,195,0.12)] hover:border-[#8A1A1A] rounded-xl p-5 flex flex-col justify-between transition-all duration-300 transform hover:-translate-y-1 cursor-pointer pointer-events-auto"
                >
                  <div>
                    <div className="flex justify-between items-center mb-4">
                      ...
                      <StatStamp percentage={paper.similarityScore || Math.floor(75 + (index * 4.3) % 23)} size="sm" />
                    </div>
                    <h3 className="text-md font-bold text-[#E9D4C3] mb-2 line-clamp-2 group-hover:text-[#F5F5F5] transition-colors">{paper.title}</h3>
